...
export default function Marketplace() {
  ...
  useEffect(() => {
    const fetchNFTs = async () => {
      setIsLoading(true);
      try {
        const fetchedNFTs = (await getAllNFTs()) as NFT[];
          ...
          fetchedNFTs.map(async (nft) => {
            try {
              const response = await fetch(
                `https://api.pinata.cloud/data/pinList?hashContains=${nft.ipfsHash}`,
                { headers: { Authorization: `Bearer ${PINATA_JWT}` } }
              );
  
              if (!response.ok) throw new Error(`HTTP error: ${response.status}`);
  
              const metadata = (await response.json())?.rows?.[0]?.metadata || {};
              ...
              return { ...nft, title: `NFT #${nft.id}`, description: "NFT Description", category: "Art", rarity: "Common", attributes: [] };
            }
          })
        );
        ...
        setAvailableCategories([...new Set(nftsWithMetadata.map((nft) => nft.category))]);
        setAvailableRarities([...new Set(nftsWithMetadata.map((nft) => nft.rarity))]);
  ...
  // Removed getAllNFTs to avoid unnecessary re-renders
