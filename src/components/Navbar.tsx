...
export function Navbar() {
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 flex items-center justify-start gap-4">

        <div className="flex items-center">
          <Link to="/" className="flex items-center gap-1 text-lg md:text-base lg:text-xl font-bold">
            <img
              src="/processed_logo.png"
              alt="NapFT"
              className="h-6 md:h-5 lg:h-7 w-auto my-0"
              loading="lazy"
            />
        ...
        <div className="flex items-center space-x-4">
          <div className={`hidden md:flex items-center relative transition-all duration-300 ${scrolled ? "w-48" : "w-52"
            }`}>
...
function NavLink({ to, children, current }: { to: string; children: React.ReactNode; current: string }) {
  ...
  return (
    <Link
      to={to}
      className={`relative px-3 py-2 rounded-md text-sm font-medium transition-all duration-200 ${isActive
          ? "text-primary"
